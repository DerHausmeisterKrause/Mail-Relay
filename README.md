...
2. `.env.example` nach `.env` kopieren (nur Bootstrap-Minimum, später GUI übernimmt):
   - initiale DB/JWT/API-Token setzen
3. Stack starten und dann **Node-/Cluster-/TLS-Settings in der Web-GUI** pflegen (`Cluster / Node Settings`):
   - `NODE_ID`, `NODE_IP`, `PEER_NODE_IP`, `VIP_ADDRESS`
   - `VRRP_PRIORITY` (A höher, B niedriger)
   - `CLUSTER_MODE` optional: `standalone`, `master`, `slave`
   - falls `slave`: `MASTER_API_URL`, `MASTER_API_TOKEN`
   - TLS Zertifikate (`tls.crt`, `tls.key`) optional direkt über GUI hinterlegen/aktualisieren
...
- Es gibt **keine Pflicht**, dass Node A/B immer Master/Slave sein müssen.
- Unterstützte Modi: `standalone` (kein Config-Pull), `master`, `slave`.
- Master verwaltet Konfiguration über GUI; jede Änderung erzeugt `config_version`.
- Ein Slave (wenn konfiguriert) pollt `MASTER_API_URL/api/config/export` (API Token) im Intervall.
- Slave übernimmt nur neuere Versionen und rendert Postfix Maps lokal.

## GUI Features

- Rollen: Admin / Operator / ReadOnly
- Cluster-/Node-Settings inkl. Mode (standalone/master/slave), VIP/VRRP und optional TLS per GUI
- Domains Whitelist
