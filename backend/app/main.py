...
from .models import User, DomainPolicy, RelayRoute, ConfigVersion, MailLog, RejectionLog, AuditLog, ClusterLock, ClusterSetting
from .auth import verify_password, hash_password, create_token, decode_token
from .schemas import LoginRequest, PasswordChangeRequest, RouteRequest, DomainRequest, ClusterSettingsRequest
...
def sync_from_master_loop():
    ...
    while True:
        try:
            ...
            mode = (cfg.get("cluster_mode") or "standalone").lower()
            if mode != "slave":
                time.sleep(interval)
                continue
...
def set_cluster_settings(req: ClusterSettingsRequest, user: User = Depends(current_user), db: Session = Depends(get_db)):
    ...
    if not row:
        row = ClusterSetting(
            node_id=req.node_id,
            node_ip=req.node_ip,
            peer_node_ip=req.peer_node_ip,
            vip_address=req.vip_address,
            vrrp_priority=req.vrrp_priority,
            cluster_mode=req.cluster_mode,
            master_api_url=req.master_api_url,
            master_api_token=req.master_api_token,
            tls_crt=req.tls_crt,
            tls_key=req.tls_key,
    ...
    if req.tls_crt and req.tls_key:
        Path('/certs/tls.crt').write_text(req.tls_crt)
        Path('/certs/tls.key').write_text(req.tls_key)

    db.add(AuditLog(actor=user.username, action='cluster_settings_updated', payload=json.dumps({
        'node_id': req.node_id,
        'node_ip': req.node_ip,
        'peer_node_ip': req.peer_node_ip,
        'vip_address': req.vip_address,
        ...
        'cluster_mode': req.cluster_mode,
        'master_api_url': req.master_api_url,
        'tls_updated': bool(req.tls_crt and req.tls_key)
    })))
    db.commit()
    return {'status':'saved','note':'restart keepalived/queue-sync containers to apply node/vrrp/env-based settings'}
