...
load_runtime() {
  ...
  NODE_ID=$(jq -r '.node_id' "$RUNTIME_JSON")
  VIP=$(jq -r '.vip_address' "$RUNTIME_JSON")
  PEER=$(jq -r '.peer_node_ip' "$RUNTIME_JSON")
  PEER_USER=$(jq -r '.peer_ssh_user // "root"' "$RUNTIME_JSON")
  [ -f /runtime/id_rsa ] && cp /runtime/id_rsa /root/.ssh/id_rsa && chmod 600 /root/.ssh/id_rsa
  [ -f /runtime/known_hosts ] && cp /runtime/known_hosts /root/.ssh/known_hosts
  return 0
}
...
safe_sync() {
  ...
  rsync -aH --delete --numeric-ids /var/spool/postfix/ "${PEER_USER}@${PEER}:/var/spool/postfix/"
...
exec 9>"$LOCK_FILE"
while true; do
  if load_runtime && is_vip_owner; then
    safe_sync || true
  fi
  sleep "$INTERVAL"
done
